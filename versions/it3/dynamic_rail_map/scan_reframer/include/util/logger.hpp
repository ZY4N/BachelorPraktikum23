...
template<typename... Args>
inline void println(
	std::ostream& out, logger::level log_level, logger::level threshold, const std::string_view& format, Args&&... args
);
...
inline static constexpr std::array<color_string, 6> colored_level_names{
	color_string{ colors::red, "error" },    color_string{ colors::yellow, "warning" },
	color_string{ colors::magenta, "info" }, color_string{ colors::white, "log" },
	color_string{ colors::blue, "debug" },   color_string{ colors::white, "generic" }
};

inline constexpr auto max_colored_level_name_length =
	std::max_element(colored_level_names.begin(), colored_level_names.end(), [](const auto& a, const auto& b) {
...
inline constexpr auto color_prefix_tuple =
	create_color_prefix_tuple(std::make_index_sequence<colored_level_names.size()>{});
...
inline constexpr auto color_prefix_lookup =
	create_color_prefix_lookup(std::make_index_sequence<colored_level_names.size()>{});

inline void print_impl(std::ostream& out, std::string_view, std::string_view format) {
	out << format;
}

template<typename First, typename... Rest>
inline void
print_impl(std::ostream& out, std::string_view color, std::string_view format, First&& first, Rest&&... rest) {
	if (format.empty()) {
		return;
	}
	...
	if (next_index == std::string::npos) {
		...
		out.write(
			logger_internal::reset_color.data(),
			static_cast<std::streamsize>(logger_internal::reset_color.size())
		);
...
instance::instance(std::ostream& n_out, std::ostream& n_err, logger::level n_level) :
...
template<typename... Args>
void println(std::ostream& out, level log_level, level threshold, const std::string_view& format, Args&&... args) {
...
instance& global_instance() {
	static auto the_global_instance = instance(std::cout, std::cerr, level::generic);
