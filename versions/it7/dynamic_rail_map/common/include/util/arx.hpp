...
template <auto ID, typename T, typename Parser> struct flag {
  using type = T;
...
/**
 * @brief function to create new flag that uses a default parser and only uses
 * a long name
 *
...
[[nodiscard]] inline constexpr arx_internal::flag<Enum, T,
                                                  arx_internal::default_parser>
make_arx_flag(std::string_view long_name);

/**
 * @brief function to create new flag that uses a default parser and has both,
...
[[nodiscard]] inline constexpr arx_internal::flag<Enum, T,
                                                  arx_internal::default_parser>
make_arx_flag(char short_name, std::string_view long_name);

/**
 * @brief function to create new flag that uses a custom parser and only uses a
 * long name
 *
 * @tparam Enum      name of the flag that has to be defined in an enum
 * @tparam T         T return type
...
[[nodiscard]] inline constexpr arx_internal::flag<Enum, T, Parser>
make_arx_flag(std::string_view long_name, Parser &&parser);

/**
 * @brief function to create new flag that uses a custom parser and has both, a
...
template <auto Enum, typename T, typename Parser>
[[nodiscard]] inline constexpr arx_internal::flag<Enum, T, Parser>
make_arx_flag(char short_name, std::string_view long_name, Parser &&parser);

using arx_ssize_t = std::make_signed_t<std::size_t>;

/**
 * @brief tamplate for a custom number of flag to use for parsing;
 *
 * @note if something has been stated multiple times, the last entry counts
 *
 * @tparam Flags  an indefinite number of flag
 */
template <typename... Flags> class arx {
private:
...
public:
  /**
   * @brief constructor to initialize the flag
   *
   * @param num_args  number of arguments
   * @param args      arguments
   * @param flags     flag to use for parsing
   */
  inline arx(int num_args, const char *const *args, Flags &&...flags);

  /**
   *@brief function to retrieve content of flag
   *
   * @tparam FlagID name of the flag whose content is to be retrieved
   * @return content of the flag
   */
  template <auto FlagID> [[nodiscard]] inline auto get() const;
   ...
   */
  [[nodiscard]] inline const std::vector<std::string_view> &
  get_positionals() const;

protected:
  [[nodiscard]] inline std::optional<std::string_view>
  find_flag_value(arx_ssize_t flag_index) const;

  template <class Flag>
  [[nodiscard]] inline std::optional<typename Flag::type>
  parse_value(Flag &flag, const std::string_view &value_str) const;

private:
  template <typename Flag>
  [[nodiscard]] inline bool matches_flag(char short_name,
                                         std::string_view long_name,
                                         std::string_view argument);

  struct bound_argument {
    inline bound_argument(const arx_ssize_t &new_flag_index,
 ...
 */
template <typename Type = int, int Base = 10,
          typename = std::enable_if_t<std::is_integral_v<Type> and (Base > 0)>>
...
[[nodiscard]] inline std::optional<Type>
floating_point(const std::string_view &str);
 ...
 */
template <typename Enum> class enumeration {
public:
  template <std::size_t Size>
  inline explicit enumeration(
      const std::array<std::pair<Enum, std::string_view>, Size> &pairs);

  template <typename... Strs>
  inline explicit enumeration(std::pair<Enum, Strs> &&...args);
   ...
   *             none correspondes an optional
   */
  [[nodiscard]] inline std::optional<Enum>
 ...
 */
template <typename T, typename Parser> class list {
public:
  /**
...
public:
   ...
   * @return     vector that contains all the elements of the list
   */
  [[nodiscard]] inline std::optional<std::vector<T>>
  operator()(const std::string_view &str) const;
...
template <typename F, typename T> struct is_parsing_function {
  static constexpr bool value =
      (std::is_same_v<std::invoke_result_t<F, std::string_view>,
                      std::optional<T>>);
};
...
template <typename... Ts> constexpr inline bool is_unique(const Ts &...values) {
  ...
  do {
    ...
    for (arx_ssize_t i = 1; i < arx_ssize_t(value_array.size()); ++i) {
      ...
      if (a == b) {
        ...
        // std::swap(a, b); // Does not run at compile time for weird reason.
        auto tmp = a;
        a = b;
        b = tmp;
        sorted = false;
...
template <arx_ssize_t Index, auto FlagID, class... Flags> struct flag_index {};

template <arx_ssize_t Index, auto FlagID, typename T, typename Parser,
          class... Flags>
...
struct flag_index<Index, FlagID, Flag, Flags...> {
  static constexpr arx_ssize_t value =
      flag_index<Index + 1, FlagID, Flags...>::value;
};

template <auto FlagID, class... Flags>
constexpr arx_ssize_t flag_index_v = flag_index<0, FlagID, Flags...>::value;

template <typename T> constexpr inline bool is_floating_from_chars(...) {
  return false;
}

template <typename T>
constexpr inline bool is_floating_from_chars(std::from_chars_result (*const)(
    const char *, const char *, T &, std::chars_format) noexcept) {
  return true;
}
...
template <typename T>
std::optional<T> extract_number(const std::string_view &str,
                                const std::string_view &suffix) {
  if (str.length() < suffix.length()) {
    return std::nullopt;
  }
  const auto num_str =
      str.substr(std::size_t(0), std::size_t(str.length() - suffix.length()));
...
std::optional<ToDuration> parse_time_from(const std::string_view &str,
                                          const std::string_view &suffix) {
  const auto number = extract_number<FromRep>(str, suffix);
  if (number.has_value()) {
    return std::chrono::duration_cast<ToDuration>(
        std::chrono::duration<FromRep, FromPeriod>(number.value()));
  }
  std::cerr << "[arx]: Ignoring invalid time value '" << str << '\''
            << std::endl;
  return std::nullopt;
}
...
std::optional<Duration> time(const std::string_view &str) {
  ...
  if (arx_parsers_internal::ends_with(str, "h")) {
    return arx_parsers_internal::parse_time_from<double, std::ratio<3'600>,
                                                 Duration>(str, "h");
  } else if (arx_parsers_internal::ends_with(str, "min")) {
    return arx_parsers_internal::parse_time_from<double, std::ratio<60>,
                                                 Duration>(str, "min");
  } else if (arx_parsers_internal::ends_with(str, "s")) {
    return arx_parsers_internal::parse_time_from<double, std::ratio<1>,
                                                 Duration>(str, "s");
  } else if (arx_parsers_internal::ends_with(str, "ms")) {
    return arx_parsers_internal::parse_time_from<std::uint64_t, std::milli,
                                                 Duration>(str, "ms");
  } else if (arx_parsers_internal::ends_with(str, "ns")) {
    return arx_parsers_internal::parse_time_from<std::uint64_t, std::nano,
                                                 Duration>(str, "ns");
  } else if (arx_parsers_internal::ends_with(str, "us")) {
    return arx_parsers_internal::parse_time_from<std::uint64_t, std::micro,
                                                 Duration>(str, "us");
  }

  std::cerr << "[arx]: Ignoring time value with invalid postfix '" << str
            << '\'' << std::endl;
...
std::optional<Type> integer(const std::string_view &str) {
  ...
  std::cerr << "[arx]: Ignoring invalid integer value '" << str << '\''
            << std::endl;
...
std::optional<Type> floating_point(const std::string_view &str) {
  ...
  std::cerr << "[arx]: Ignoring invalid floating point value '" << str << '\''
            << std::endl;
...
template <typename... Strs>
enumeration<Enum>::enumeration(std::pair<Enum, Strs> &&...args)
    : m_name_value_pairs{args...} {}

template <typename Enum>
template <std::size_t Size>
enumeration<Enum>::enumeration(
    const std::array<std::pair<Enum, std::string_view>, Size> &pairs)
    : m_name_value_pairs{pairs.begin(), pairs.end()} {}

template <typename Enum>
std::optional<Enum>
enumeration<Enum>::operator()(const std::string_view &str) const {
  ...
  std::cerr << "[arx]: Ignoring invalid enum value '" << str << '\''
            << std::endl;
...
template <typename T, typename Parser>
list<T, Parser>::list(Parser &&parser)
    : m_parse_value{std::forward<Parser>(parser)} {}

template <typename T, typename Parser>
std::optional<T>
list<T, Parser>::parse_value(const std::string_view &value_str) const {
  ...
  if constexpr (not std::is_same_v<Parser, arx_internal::default_parser>) {
    ...
    try {
      ...
      std::cerr << "[arx]: Ignoring invalid path '" << value_str << '\''
                << std::endl;
      return std::nullopt;
    }
...
list<T, Parser>::operator()(const std::string_view &str) const {

  const auto num_values = std::accumulate(str.begin(), str.end(), 1ull,
                                          [](const auto &count, const char &c) {
  ...
  while (true) {
    ...
    const auto value_str = str.substr(begin_index, end_index - begin_index);
    auto value_opt = parse_value(value_str);
    if (value_opt) {
      values.emplace_back(std::move(value_opt).value());
    } else {
      std::cerr << "[arx]: Ignoring invalid list item '" << value_str << '\''
                << std::endl;
    }
...
template <typename T> list<T, arx_internal::default_parser> make_list() {
...
constexpr arx_internal::flag<Enum, T, arx_internal::default_parser>
make_arx_flag(char short_name, std::string_view long_name) {
...
template <auto Enum, typename T, typename Parser>
constexpr arx_internal::flag<Enum, T, Parser>
make_arx_flag(char short_name, std::string_view long_name, Parser &&parser) {
  static_assert(std::is_enum_v<decltype(Enum)>, "Flag ID must have enum type.");
  static_assert(arx_internal::is_parsing_function_v<Parser, T>,
...
template <typename Flag>
bool arx<Flags...>::matches_flag(char short_name, std::string_view long_name,
                                 std::string_view argument) {
  using namespace arx_internal;
  return (short_name != no_short_flag and
          argument.length() ==
              short_flag_prefix.length() + sizeof(short_name) and
          argument.substr(0, short_flag_prefix.length()) ==
              short_flag_prefix and
          argument[short_flag_prefix.length()] == short_name) or
         (argument.length() ==
              long_flag_prefix.length() + long_name.length() and
...
arx<Flags...>::arx(int num_args, const char *const *args, Flags &&...flags)
    ...
    const auto found_match = std::apply(
        [&](const Flags &...flag_config) {
          return ((++flag_index,
                   ...
                   matches_flag<Flags>(flag_config.short_name,
                                       flag_config.long_name, argument)) or
                  ...);
        },
        m_flags);
  ...
  // duplicates.
  std::stable_sort(m_bound_arguments.begin(), m_bound_arguments.end(),
                   [](const bound_argument &a, const bound_argument &b) {
                     // For some reason '<' confuses clang 16.
                     return std::less<arx_ssize_t>{}(a.flag_index,
                                                     b.flag_index);
                   });

  // Only store last duplicate.
  const auto last =
      std::unique(m_bound_arguments.rbegin(), m_bound_arguments.rend(),
                  [](const bound_argument &a, const bound_argument &b) {
...
arx<Flags...>::find_flag_value(arx_ssize_t flag_index) const {
  // Binary search through flag IDs.
  const auto it = std::lower_bound(
      m_bound_arguments.begin(), m_bound_arguments.end(), flag_index,
      [](const bound_argument &a, const arx_ssize_t &index) {
        return a.flag_index < index;
      });
...
template <class... Flags>
template <class Flag>
std::optional<typename Flag::type>
arx<Flags...>::parse_value(Flag &flag,
...
arx<Flags...>::get_positionals() const {
