...
std::error_code drm::unbagger::extract_transforms(
	...
	const auto origin_ignore_radius_sq = std::pow(origin_ignore_radius, 2.0);
	...
	for (const auto& msg : transforms_view) {
		...
		const auto dist_from_origin_sq = pose.translation.squaredNorm();
		if (dist_from_origin_sq <= origin_ignore_radius_sq) {
			logger::warn(
				"Ignoring transform with coordinates (%m, %m, %m) because it falls into the origin ignore radius.",
				pose.translation.x(),
				pose.translation.y(),
				pose.translation.z()
			);
		...
		transforms[num_transforms++] = { pose, ros_time_to_timestamp_t(first_transform.header.stamp) };
	}

	if (num_transforms != transforms.size()) {
		logger::warn("Skipped % out of % transforms messages.", transforms.size() - num_transforms, transforms.size());
...
std::error_code drm::unbagger::extract_velocities(
	...
	if (num_velocities != velocities.size()) {
		logger::warn("Skipped % out of % odometer messages.", velocities.size() - num_velocities, velocities.size());
...
std::error_code drm::unbagger::extract_imu_data(
	...
	if (num_rotations != rotations.size()) {
		logger::warn("Skipped % out of % imu messages.", rotations.size() - num_rotations, rotations.size());
		// Shrink vector to size without duplicates
