...
void test_error(
	...
	const auto emulated_error_string = std::apply([&](auto&&... args) { return emulate_error(args...); }, emulate_args);
	const auto logger_error_string = std::apply(
		[&](auto&&... args) { return error_to_string(threshold, args...); },
		logger_args
	);
...
void test_warn(
	...
	const auto emulated_warn_string = std::apply([&](auto&&... args) { return emulate_warn(args...); }, emulate_args);
	const auto logger_warn_string = std::apply(
		[&](auto&&... args) { return warn_to_string(threshold, args...); },
		logger_args
	);
...
void test_info(
	...
	const auto emulated_info_string = std::apply([&](auto&&... args) { return emulate_info(args...); }, emulate_args);
	const auto logger_info_string = std::apply(
		[&](auto&&... args) { return info_to_string(threshold, args...); },
		logger_args
	);
...
void test_log(
	...
	const auto emulated_log_string = std::apply([&](auto&&... args) { return emulate_log(args...); }, emulate_args);
	const auto logger_log_string = std::apply(
		[&](auto&&... args) { return log_to_string(threshold, args...); },
		logger_args
	);
...
void test_debug(
	...
	const auto emulated_debug_string = std::apply([&](auto&&... args) { return emulate_debug(args...); }, emulate_args);
	const auto logger_debug_string = std::apply(
		[&](auto&&... args) { return debug_to_string(threshold, args...); },
		logger_args
	);
...
TEST(logger_test, format_only_text) {
	test_all_levels(std::make_tuple("Hello World"), std::make_tuple("Hello World"));
}

TEST(logger_test, format_only_value) {
	test_all_levels(std::make_tuple("%", 8), std::make_tuple(make_arg_value(8)));
}
...
TEST(logger_test, format_allows_ios_parameters) {
	test_all_levels(
		...
		std::make_tuple(
			"This includes 0x",
			make_arg_value(std::hex),
			make_arg_value(42),
			" 0o",
			make_arg_value(std::oct),
			make_arg_value(42),
			" and 0b",
			make_arg_value(std::bitset<32>{ 42 }),
			"."
		)
	);
}
