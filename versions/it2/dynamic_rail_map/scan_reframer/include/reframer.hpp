 ...
 * @param timed_transforms                A by timestamp ascendingly sorted range of timed tram to destination space transforms .
...
} // namespace point_remapper


namespace reframer_internal {

template<typename T>
[[nodiscard]] double calculate_t(const T& p0, const T& p1);

template<typename T>
[[nodiscard]] T eval_catmull_rom_spline(
	const T& p0, const T& p1, const T& p2, const T& p3,
	double dist_01, double dist_12, double dist_23,
	double t
);
...
[[nodiscard]] double advance_and_sample_velocity(
	tcb::span<const types::timed_velocity_t>::iterator& velocity_it,
	const types::timestamp_t& timestamp
);

[[nodiscard]] types::lidar_frame_t reframe_points_to_pose(
	tcb::span<types::timed_point_t> points,
...
[[nodiscard]] std::error_code make_error_code(codes e);

} // namespace reframer_error

[[nodiscard]] inline std::error_category& reframer_error_category();
