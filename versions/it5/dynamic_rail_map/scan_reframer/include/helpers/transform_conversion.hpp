...
/**
 * @brief Calculates the Euler Angels for a @c types::types::matrix_t.
 *
 * This function calculates Euler Angels for a @c types::types::matrix_t and writes the angels to the given variable references.
 *
 * @tparam transform	The transformation matrix
 *
 * @param t1  			Reference to angle_x.
 * @param t2  			Reference to angle_y.
 * @param t3  			Reference to angle_z.
 *
 * @return             A @c types::transform_t representing the constructed transformation.
 */
template<typename T>
inline void extractEulerAngleXYZ(const drm::types::matrix_t& transform, T& t1, T& t2, T& t3) noexcept;
