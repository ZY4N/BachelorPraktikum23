...
if validatePogramm(
        f"./drm_input_node {rosbag_path} -o /tmp/map_data --max-radial-dist 30 --max-peripheral-dist 20 --time-per-block 2min --max-scan-pos-deviation 1 --chunk-size 30 --frame-format uos") \
        and validatePogramm(
    f"{pprm} -j10 --voxel-size=10 --maxrange-method=normals --normal-method=angle -f uos /tmp/map_data/chunk0_0/frames") \
        and meta_json("/tmp/map_data") \
        and clean() \
        and validatePogramm(
    f"./drm_input_node {rosbag_path} --max-radial-dist 30 --max-peripheral-dist 20 --time-per-block 2min --max-scan-pos-deviation 1 --chunk-size 30 --frame-format kitti") \
        and validatePogramm("roslaunch removert run_kitti.launch") \
        and meta_json("/tmp/map_data") \
        and validatePogramm("/home/sera/CLionProjects/dynamic_Rail_Map/kitti2pcd/cmake-build-debug/kitti2pcd -r /tmp/map_data")\
        and clean():
